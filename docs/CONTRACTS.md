...
| `GET /probes/{request_id}` | durable probe state (read-only): `state`, `pending`, result `status`, typed result document, `observation_applied`; subject-bound: readable only by the authenticated subject recorded at enqueue (normalized owner identity), so a mismatched or legacy-empty subject gets the same 404 as an unknown probe; a stored result is bounded to `PROBE_RESULT_MAX_BYTES` (200000) serialized UTF-8 bytes and is never truncated: an over-bound result is stored as `{reason: probe_result_too_large, max_bytes, actual_bytes}` with `status: error`, and a serialization failure as `{reason: probe_result_serialization_failed}` with `status: error` |
...
| `GET /health` | authenticated API/DB/worker readiness; worker liveness from `<state_dir>/dispatcher.heartbeat` file (fresh ≤20s), never inferred from job absence; `drain` reports the actual `<state_dir>/drain` marker directly (read-only, independent of api/database/worker/recovery_required; drain is not a failure) |
